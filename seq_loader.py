...
class TrainData(torch.utils.data.DataLoader):
    ...
    def __getitem__(self, idx):
        ...
        # heatmaps = np.load(self.data_path + "heatmaps_train/" + format(idx + 1, '03d') + ".npy")
        label = self.labels[idx]

        img = np.moveaxis(img, -1, 0)
        img = torch.from_numpy(img).float()

        return img, hand_poses, label, obj_poses#, heatmaps
...
class ValData(torch.utils.data.DataLoader):
    ...
    def __getitem__(self, idx):
        ...
        heatmaps = np.load(self.data_path + "heatmaps_val/" + format(idx + 1, '03d') + ".npy")
        label = self.labels[idx]
        img = np.moveaxis(img, -1, 0)
        img = torch.from_numpy(img).float()
